...
<script lang="ts">
	...
	function getBattleStatusColor(status: string) {
		switch (status) {
			...
			default:
				return "bg-[#8a8272]/20 border-[#8a8272]/30 text-[#a89e8e]";
		}
	}
		...
		<div class="w-full px-4 sm:px-6 py-4 sm:py-6">
			...
			<div class="flex items-center justify-between mb-4">
				<div class="flex items-center gap-2">
					...
					{:else}
						<div class="size-2.5 bg-[#8a8272] rounded-full"></div>
						<span class="text-[#a89e8e] font-mono text-xs uppercase tracking-widest">War Ended</span>
					{/if}
				</div>
				<span class="text-[#a89e8e]/70 font-mono text-xs">WAR #{data.war.id}</span>
			</div>
				...
				<a href="/state/{data.war.attacker.id}" class="group flex flex-col items-center gap-3 text-center">
					...
					<div>
						...
						{#if data.war.attackerBloc}
							<div class="text-xs text-[#a89e8e] font-mono">{data.war.attackerBloc.name}</div>
						{/if}
					</div>
				</a>
					...
					{#if data.war.status === "active"}
						<div class="font-mono text-center">
							<div class="text-2xl sm:text-3xl font-bold text-white">
								{warDuration.days}<span class="text-[#a89e8e]/70 text-lg">d</span>
								{String(warDuration.hours).padStart(2, "0")}<span class="text-[#a89e8e]/70 text-lg">h</span>
							</div>
							<div class="text-xs text-[#a89e8e] uppercase tracking-wider mt-1">Duration</div>
						</div>
					{:else}
						<div class="font-mono text-center">
							<div class="text-lg font-bold text-[#a89e8e]">
								{warDuration.days}d {warDuration.hours}h
							</div>
							<div class="text-xs text-[#a89e8e]/70 uppercase tracking-wider mt-1">Total Duration</div>
						</div>
					{/if}
				</div>
				...
				<a href="/state/{data.war.defender.id}" class="group flex flex-col items-center gap-3 text-center">
					...
					<div>
						...
						{#if data.war.defenderBloc}
							<div class="text-xs text-[#a89e8e] font-mono">{data.war.defenderBloc.name}</div>
						{/if}
					</div>
				</a>
			</div>
	...
	<div class="w-full px-4 sm:px-6 py-4 sm:py-6 space-y-4 sm:space-y-6">
		...
		<div class="bg-[#14283f]/85 border border-[#dfceb0]/15 rounded-xl p-4 sm:p-6">
			<div class="flex items-center justify-between mb-3">
				...
				<div class="text-xs text-[#a89e8e] font-mono uppercase tracking-wide">Territory Control</div>
				<div class="flex items-center gap-2">
			...
			<div class="relative h-8 sm:h-10 bg-[#0d1d31]/90 rounded-lg border border-[#dfceb0]/15 overflow-hidden">
				<div
		...
		{#if data.capitulatedStates && data.capitulatedStates.length > 0}
			<div class="bg-gradient-to-r from-red-950/30 to-[#0d1d31]/30 border border-red-500/30 rounded-xl p-4 sm:p-5">
				...
				<div class="flex flex-wrap gap-3">
					{#each data.capitulatedStates as state}
						<div class="flex items-center gap-2 bg-[#14283f]/70 border border-red-500/20 rounded-lg px-3 py-2">
							...
							<div>
								...
								{#if state.capitulatedAt}
									<div class="text-[10px] text-[#a89e8e] font-mono">{formatDate(state.capitulatedAt)}</div>
								{/if}
							</div>
						</div>
					{/each}
				...
				<div class="p-3 sm:p-4 space-y-2">
					{#each ongoingBattles as battle}
						<a
							...
							class="flex items-center gap-3 sm:gap-4 bg-[#102239]/60 border border-amber-500/20 rounded-lg p-3 sm:p-4 hover:border-amber-400/40 transition-all group"
						>
							<Logo
			...
			>
				...
				<div class="p-3 sm:p-4 space-y-2 max-h-[32rem] overflow-y-auto">
					{#each completedBattles as battle}
						...
						<a
							href="/battle/{battle.id}"
							class="flex items-center gap-3 sm:gap-4 bg-[#102239]/60 border border-[#dfceb0]/12 rounded-lg p-3 sm:p-4 hover:border-[#dfceb0]/22 transition-all group"
						>
							<Logo
								src="/coats/{battle.region.id}.svg"
								alt={getRegionName(battle.region.id)}
							...
							<div class="flex-1 min-w-0">
								<div
									class="text-sm sm:text-base font-bold text-white group-hover:text-[#e5d8c1] transition-colors truncate"
								>
									{getRegionName(battle.region.id)}
								</div>
								<div class="text-xs text-[#a89e8e] font-mono">
		...
		{:else if totalBattles === 0}
			<div class="bg-[#102239]/50 border border-[#dfceb0]/10 rounded-xl p-8 sm:p-12 text-center">
				...
				<p class="text-lg text-[#a89e8e] font-mono">No battles fought yet</p>
			</div>
		{/if}
			...
			>
				<div class="bg-[#102239]/90 border-b border-[#dfceb0]/15 px-4 sm:px-6 py-3 sm:py-4">
					<div class="flex items-center gap-2">
						...
						<h3 class="text-base sm:text-lg font-bold text-[#fff7e8] font-mono uppercase tracking-wide">Surrenders</h3>
					</div>
				</div>
				<div class="p-3 sm:p-4 space-y-2">
					{#each data.war.surrenders as surrender}
						<div class="flex items-center gap-3 bg-[#102239]/60 border border-[#dfceb0]/12 rounded-lg p-3 sm:p-4">
							<FluentFlag20Filled class="size-5 text-[#a89e8e] flex-shrink-0" />
							<div class="flex-1 min-w-0">
