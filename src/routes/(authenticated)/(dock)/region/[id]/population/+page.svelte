...
<PageContainer maxWidth="6xl">
	...
	<div class="flex items-center justify-between gap-3">
		...
		<button
			onclick={toggleSort}
			class="btn btn-sm bg-slate-700 hover:bg-slate-600 border-0 text-white gap-1.5 shrink-0"
		>
			...
			{#each data.residents as resident}
				...
				>
					<div
						...
						{isYou ? 'ring-blue-500/30 group-hover:ring-blue-500/50' : 'ring-white/5 group-hover:ring-white/10'}"
					>
						<Logo
					...
					<div class="flex-1 min-w-0">
						<p
							...
							{isYou ? 'text-blue-300 group-hover:text-blue-200' : 'text-white group-hover:text-purple-400'}"
						>
					...
					<FluentChevronRight20Filled
						class="size-4 text-gray-600 group-hover:text-purple-400 transition-colors shrink-0"
					/>
				</a>
			{/each}
		</div>
