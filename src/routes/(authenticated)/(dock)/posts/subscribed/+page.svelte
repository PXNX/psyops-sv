    ...
    <div class="w-full mx-auto px-3 sm:px-4 py-4 sm:max-w-4xl">
        <div class="flex items-center gap-3">
            <button
                ...
                title="Go back"
            >
                <FluentArrowLeft20Filled class="size-5 text-gray-300" />
            </button>

            <h1 class="text-xl font-bold text-white hidden sm:block">Subscribed</h1>
    ...
    {#if filteredArticles.length === 0}
        <div class="text-center py-16">
            <div class="inline-flex items-center justify-center size-20 bg-slate-800/50 rounded-full mb-4">
                <FluentEmojiRolledUpNewspaper class="text-4xl" />
            </div>
            <p class="text-lg text-gray-400">
                {searchQuery ? "No posts found matching your search" : "No posts from subscribed newspapers yet"}
            </p>
            {#if !searchQuery}
                <button
                    onclick={() => goto("/newspaper")}
                    class="btn btn-sm gap-2 bg-blue-600/20 hover:bg-blue-600/30 border-blue-500/30 text-blue-300 hover:text-blue-200 mt-4"
                >
