...
export const stateFormationStatusEnum = pgEnum("state_formation_status", [
	"pending",
	"active",
	"completed",
	"cancelled"
...
// State Formation Periods table
export const stateFormationPeriods = pgTable("state_formation_periods", {
	id: uuid("id").defaultRandom().primaryKey(),
	regionId: integer("region_id")
		.notNull()
		.references(() => regions.id, { onDelete: "cascade" })
	...
	initiatedBy: text("initiated_by")
		.notNull()
		.references(() => accounts.id, { onDelete: "cascade" }),
	proposedStateName: varchar("proposed_state_name", { length: 100 }).notNull(),
	status: stateFormationStatusEnum("status").notNull().default("pending"),
	startedAt: timestamp("started_at").defaultNow().notNull(),
	endsAt: timestamp("ends_at").notNull(), // 3 days from start
	createdAt: timestamp("created_at").defaultNow().notNull()
});

// State Formation Proposals table (for competing state name proposals)
export const stateFormationProposals = pgTable("state_formation_proposals", {
	id: uuid("id").defaultRandom().primaryKey(),
	formationPeriodId: uuid("formation_period_id")
		.notNull()
		.references(() => stateFormationPeriods.id, { onDelete: "cascade" }),
	...
	description: text("description"),
	logo: text("logo"),
	mapColor: varchar("map_color", { length: 7 }).default("#6366f1").notNull(),
	voteCount: integer("vote_count").default(0).notNull(),
	createdAt: timestamp("created_at").defaultNow().notNull()
});

// State Formation Votes table
export const stateFormationVotes = pgTable(
	"state_formation_votes",
	{
		id: uuid("id").defaultRandom().primaryKey(),
		proposalId: uuid("proposal_id")
			.notNull()
			.references(() => stateFormationProposals.id, { onDelete: "cascade" }),
	...
	(table) => ({
		proposalVoterIdx: uniqueIndex("idx_formation_proposal_voter").on(table.proposalId, table.voterId)
	})
);

// Relations
export const stateFormationPeriodsRelations = relations(stateFormationPeriods, ({ one, many }) => ({
	region: one(regions, {
		fields: [stateFormationPeriods.regionId],
		references: [regions.id]
	}),
	initiator: one(accounts, {
		fields: [stateFormationPeriods.initiatedBy],
		references: [accounts.id]
	}),
	proposals: many(stateFormationProposals)
}));

export const stateFormationProposalsRelations = relations(stateFormationProposals, ({ one, many }) => ({
	formationPeriod: one(stateFormationPeriods, {
		fields: [stateFormationProposals.formationPeriodId],
		references: [stateFormationPeriods.id]
	}),
	proposer: one(accounts, {
		fields: [stateFormationProposals.proposedBy],
		references: [accounts.id]
	}),
	votes: many(stateFormationVotes)
}));

export const stateFormationVotesRelations = relations(stateFormationVotes, ({ one }) => ({
	proposal: one(stateFormationProposals, {
		fields: [stateFormationVotes.proposalId],
		references: [stateFormationProposals.id]
	}),
	voter: one(accounts, {
		fields: [stateFormationVotes.voterId],
		references: [accounts.id]
	})
}));

// TypeScript types
export type StateFormationPeriod = typeof stateFormationPeriods.$inferSelect;
export type NewStateFormationPeriod = typeof stateFormationPeriods.$inferInsert;

export type StateFormationProposal = typeof stateFormationProposals.$inferSelect;
export type NewStateFormationProposal = typeof stateFormationProposals.$inferInsert;

export type StateFormationVote = typeof stateFormationVotes.$inferSelect;
export type NewStateFormationVote = typeof stateFormationVotes.$inferInsert;
