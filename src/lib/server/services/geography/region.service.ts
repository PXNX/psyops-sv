...
import { regions, residences, regionBorders } from '../../schema';
...
export class RegionService {
     ...
     */
    async getBorderingRegions(regionId: number): Promise<Array<{ id: number; distanceKm: number }>> {
        // Since we store borders in both directions (region_id < neighbor_id),
        // we need to query both ways
        const borders = await this.db
            .select({
     ...
     */
    async areRegionsAdjacent(regionId1: number, regionId2: number): Promise<boolean> {
        const [smaller, larger] = regionId1 < regionId2 ? [regionId1, regionId2] : [regionId2, regionId1];

        const border = await this.db.query.regionBorders.findFirst({
     ...
     */
    async getBorderDistance(regionId1: number, regionId2: number): Promise<number | null> {
        const [smaller, larger] = regionId1 < regionId2 ? [regionId1, regionId2] : [regionId2, regionId1];
     ...
     */
    async getStateBorderingRegions(
        stateId: number,
        targetRegionId: number
    ): Promise<Array<{ id: number; name: string; distanceKm: number }>> {
        // Get all regions of the state
        const stateRegions = await this.db.query.regions.findMany({
     ...
     */
    calculateDirectDistance(lat1: number, lon1: number, lat2: number, lon2: number): number {
     ...
     */
    async getDistanceBetweenRegions(regionId1: number, regionId2: number): Promise<number | null> {
        // First try to get border distance if they're adjacent
        const borderDistance = await this.getBorderDistance(regionId1, regionId2);
        if (borderDistance) {
            return borderDistance;
        }

        // Otherwise calculate direct distance
        const region1 = await this.db.query.regions.findFirst({
            where: eq(regions.id, regionId1)
        });
        const region2 = await this.db.query.regions.findFirst({
        ...
        return this.calculateDirectDistance(
